...
def test_find_experiment_dirs_treats_flat_run_dir_as_single_run_experiment(tmp_path):
    # backwards compat with the checked-in flat samples (data/AND, data/XOR, ...): a folder
    # with no run subfolders that is itself a run (overview file directly inside it).
    data_root = tmp_path / "data"
    data_root.mkdir()
    flat = data_root / "AND"
    flat.mkdir()
    (flat / "per_generation_overview.txt").write_text(_overview_line(0, 1, 0.5))

    experiments = find_experiment_dirs(data_root)
    assert [name for name, _ in experiments] == ["AND"]

    runs = find_runs_with_overview(dict(experiments)["AND"])
    assert [(name, path) for name, path in runs] == [("AND", flat)]
