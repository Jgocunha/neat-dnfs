...
namespace neat_dnfs
{
	...
	void SelectTheObject::testPhenotype()
	{
		...
		static constexpr double w0 = 1 / 4.f;
		static constexpr double w1 = 1 / 4.f;
		static constexpr double w2 = 1 / 4.f;
		static constexpr double w3 = 1 / 4.f;
		...
		const double f0 = twoBumpsAtPositionWithAmplitudeAndWidth("nf 1",
						20.0, in_amp, in_width,
						80.0, in_amp, in_width);
		parameters.fitness = w0 * f0;

		const double f1 = justOneBumpAtOneOfTheFollowingPositionsWithAmplitudeAndWidth("nf 4", { 20.0, 80.0 }, out_amp, out_width);
		parameters.fitness += w1 * f1;

		addGaussianStimulus("nf 3",
							{ GaussStimulusConstants::width, GaussStimulusConstants::amplitude, 50.0, true, false },
							{ DimensionConstants::xSize, DimensionConstants::dx });
		runSimulation(iterations);
		const double f2 = justOneBumpAtOneOfTheFollowingPositionsWithAmplitudeAndWidth("nf 4", { 50.0 }, out_amp, out_width);
		parameters.fitness += w2 * f2;
