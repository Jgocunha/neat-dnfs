...
namespace neat_dnfs
{
	...
	void ColorSpaceMapOutputSustainedSolution::testing()
	{
		...
		for (const auto& inputOutputBump : inputOutputBumps)
		{
			...
			const double f1 = oneBumpAtPositionWithAmplitudeAndWidth("nf 1", expectedInput, 20.0, 17.0);
			const double f2 = oneBumpAtPositionWithAmplitudeAndWidth("nf 2", expectedOutput, 20.0, 17.0);
		...
		std::cout << "Final fitness: " << parameters.fitness << std::endl;
	}
