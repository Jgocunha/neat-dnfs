...
namespace neat_dnfs
{
	...
	void Population::reassignOffspringIfFitnessIsStagnant() const
	{
		...
		log(tools::logger::LogLevel::WARNING, "Reassigned " + std::to_string(totalOffspringToReassign)
			+ " offspring to species " + std::to_string(topSpecies->getId()) + ".");
	}
			...
			for (const auto& solution_b : solutions)
			{
				...
				for (const auto& connectionGene_a : connectionGenes_a)
				{
					for (const auto& connectionGene_b : connectionGenes_b)
					{
						...
						{
							log(tools::logger::LogLevel::FATAL, "Couplings are the same.");
						}
					}
				}
	...
	void Population::startKeyListenerForUserCommands()
	{
		std::thread keyListener([this]() {
			while (!control.stop)
			{
				...
				{
					control.stop = true;
					tools::logger::log(tools::logger::LogLevel::INFO, "Stopping evolution "
