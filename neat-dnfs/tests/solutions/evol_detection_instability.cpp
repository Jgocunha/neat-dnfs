...
TEST_CASE("DetectionInstability evolves without crashing, without violating invariants, and reports consistent fitness statistics",
    "[Evolution][Solutions][DetectionInstability]")
{
    std::vector<EvolutionRunResult> results;
    REQUIRE_NOTHROW(results = runEvolution<DetectionInstability>(makeTopology(1, 1)));
