...
TEST_CASE("InhibitionOfReturn evolves without crashing, without violating invariants, and reports consistent fitness statistics",
    "[Evolution][Solutions][InhibitionOfReturn]")
{
    std::vector<EvolutionRunResult> results;
    REQUIRE_NOTHROW(results = runEvolution<InhibitionOfReturn>(makeTopology(1, 1)));
