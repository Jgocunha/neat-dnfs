...
TEST_CASE("MemoryInstability evolves without crashing, without violating invariants, and reports consistent fitness statistics",
    "[Evolution][Solutions][MemoryInstability]")
{
    std::vector<EvolutionRunResult> results;
    REQUIRE_NOTHROW(results = runEvolution<MemoryInstability>(makeTopology(1, 1)));
